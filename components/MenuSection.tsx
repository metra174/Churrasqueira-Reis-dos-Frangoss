...
const CATEGORY_IMAGES: Record<string, string> = {
  entradas_frangos: 'https://images.unsplash.com/photo-1598515214211-89d3c73ae83b?auto=format&fit=crop&w=1920&q=80',
  compostos: 'https://images.unsplash.com/photo-1544025162-d76694265947?auto=format&fit=crop&w=1920&q=80',
...
const MenuItemRow: React.FC<{ item: MenuItem, onAdd: (item: MenuItem) => void }> = ({ item, onAdd }) => (
  <div className="flex justify-between items-center py-5 border-b border-white/10 hover:bg-white/5 px-6 transition-all duration-300 group rounded-xl">
    <div className="flex flex-col flex-1 mr-4">
      ...
      {item.description && (
        <span className="text-gray-400 text-xs mt-1.5 font-light italic max-w-lg leading-relaxed">
          {item.description}
        </span>
      )}
    </div>
  ...
  <section id={category.id} className="relative min-h-screen flex flex-col items-center justify-center py-32 overflow-hidden border-b border-white/5">
    ...
    <div className="container mx-auto px-6 max-w-5xl relative z-10">
      <div className="text-center mb-16 animate-fade-in-up">
        <span className="text-gold font-bold uppercase tracking-[0.5em] text-[10px] bg-black/60 px-6 py-2 rounded-full border border-white/10 mb-6 inline-block backdrop-blur-md">
          REIS NO SABOR
        </span>
